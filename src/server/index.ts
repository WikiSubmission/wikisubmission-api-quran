...
export class Server {
  ...
  async registerRoutes() {
    ...
    this.server.log.info(
      `${routes.length} routes: ${routes.map((r) => `${r.url}`).join(", ")}`,
    );
    for (const route of routes) {
      try {
        // Add caching interceptor if route has cache configuration
        if (route.cache) {
          const cacheDuration = this.convertCacheDuration(
          ...
          route.handler = async (request, reply) => {
            ...
            if (
              cacheEntry &&
              now - cacheEntry.timestamp < cacheDuration * 1000
            ) {
  ...
  async registerPlugins() {
    ...
    this.server.register(fastifyCaching, {
